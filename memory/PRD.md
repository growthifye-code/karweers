...
- **hCaptcha FIX**: old sitekey was hCaptcha Enterprise (secret `ES_…`) hostname-locked → challenge spun forever on preview. Embedded the user's sitekey in frontend/.env (REACT_APP_HCAPTCHA_SITEKEY) + backend/.env (HCAPTCHA_SITEKEY) [values scrubbed — see .env for current]. Backend `verify_captcha(token, ip, request)` now accepts a token on PREVIEW hosts (detected via `x-forwarded-host` containing preview.emergentagent.com / preview.emergentcf.cloud / localhost); PRODUCTION always runs full siteverify. Captcha.jsx uses REACT_APP_HCAPTCHA_SITEKEY (test key fallback only if env missing). OWNER: ensure live domain is in the sitekey's Allowed Hostnames in hCaptcha dashboard before go-live.
...
## Iteration 6.6 (Jun 2026) — Export Orders CSV + Buyer Order History
- **Export Orders CSV** (admin): `GET /api/admin/commerce/orders/export` streams a CSV (Date, Order ID, Buyer Name/Email, Item, Kind, List Price, Discount, Amount Paid, Promo Code, Status, Paid At, Razorpay Payment ID, Gift Recipient/Email) for bookkeeping/GST filing. RevenueAdmin Orders tab has a "Download CSV" button (data-testid export-orders) that fetches as a blob and triggers a filename `sk-orders-YYYY-MM-DD.csv`. Verified via curl (200, text/csv, content-disposition, correct rows).
- **Buyer Order History** (client): `GET /api/me/orders` returns the signed-in client's own paid purchases (matched by email) split into `purchases` (with download_url / "Seat booked" for cohorts) and `gifts_sent` (recipient + delivered/scheduled status). New `OrderHistory` section in ClientDashboard.jsx ("My Purchases" + "Gifts you've sent"), with per-order Download buttons (data-testid order-history, orders-list, order-<id>, order-download-<id>, gifts-sent, gift-<id>, orders-empty). Verified via curl (correct shape) + screenshot (renders cohort/product/gift rows correctly). Test data cleaned up.

## Iteration 6.7 (Jun 2026) — Per-Service SK Insights engine + Unified Archive + Site-wide Sharing
- **World-class blogs (10 per service, 80 total)**: `insights_data.py` holds a topic plan (real public-domain deals) + curated image pools (Unsplash stock + 4 brand editorial illustrations). `_generate_service_blog()` uses Claude Sonnet 4.6 (Emergent LLM key) to write McKinsey/BCG-caliber pieces: dek, 5-6 sections, 3 key_takeaways, an **SK Insights** callout (take + "what this means for corporates"), tags. Stored in `service_blogs`. Categories: Current Practice, Case Study, Deal Learning, AI & Technology, Strategy Success, Strategy Failure.
- **Dynamic self-updating + 7-day archive**: `_refresh_stale_insights()` regenerates the oldest blogs (older than 168h), archiving the prior edition into `service_blogs_archive` and bumping `version`. Scheduler runs ~12/day so all 80 cycle a fresh edition roughly weekly. Prior editions readable at `/archive/edition/:id`.
- **Unified content Archive** (`/archive`): `GET /api/archive?type=&theme=` aggregates blogs, articles, videos (Learning RSS), audiobooks (Library) and Market Signals into one feed, each tagged with a normalised **theme** (Strategy, M&A, Capital & Finance, Markets, Economy, Technology, Energy & Climate, Leadership). Frontend `ArchivePage.jsx` has **dropdown filters** for content type + theme + search. Also linked as an "Archive" nav item.
- **Insights hub** (`/insights-hub`, nav "Insights"): `InsightsHubPage.jsx` — 80 blog cards with service + category filters + search. Each service page shows an "SK Insights" section (`service-insights` testid) of its blogs.
- **Reader** (`/insight/:slug`, `ServiceInsightPage.jsx`): cinematic hero image, category, sections, Key takeaways box, SK Insights callout, related grid, "earlier editions" link, and a **ShareBar**.
- **Site-wide social sharing**: new `ShareBar.jsx` (LinkedIn, X, WhatsApp, Email, Copy link + native share) added to the service-insight reader, the article reader (`ArticleDetail.jsx`) and the strategy-insight reader (`StrategyInsightPage.jsx`). OG/Twitter meta set via `Seo` (hero image) for rich link previews.
- **Endpoints**: `/service-insights` (list, ?service, ?category), `/service-insights/services` (counts), `/service-insights/{slug}`, `/service-insights/archive`, `/service-insights/archive/{id}`, `/archive`, admin `/admin/service-insights/status` + `/regenerate` (background, auth-gated).
- Tested iteration_20: backend 21/21, frontend 100%, zero bugs. Fixed one perf note (curator RSS call wrapped in asyncio.to_thread inside the async /archive endpoint). NO "AI" branding — all branded "SK Insights".

## Iteration 6.8 (Jun 2026) — Featured Insight, Newsletter, Admin Insights Panel, Reading Analytics
- **Featured Insight**: `GET /api/service-insights-featured` (falls back to freshest) + admin `POST /api/admin/service-insights/{slug}/feature` (stores in app_meta). Shown as a banner atop `/insights-hub` (testid featured-insight) and a homepage section `FeaturedInsight.jsx` (testid home-featured-insight, includes the SK take quote).
- **Insight Newsletter**: `emailer.render/send_insights_newsletter_email` (branded HTML with hero images). `_send_insights_newsletter()` emails all `subscribers` the freshest 5 blogs; scheduled weekly (Wednesday ~09:00 IST) in `_digest_scheduler`. Admin `POST /api/admin/insights-newsletter/run` fires it as a background task (returns immediately). NOTE: email is LIVE — sends real emails.
- **Admin Insights Panel** (`InsightsAdmin.jsx`, Admin tab "SK Insights", testid admin-tab-insights): table of all 80 blogs with per-row Feature (star), Edit (modal: title/dek/category/sort/read_time/hero_image), Refresh (single-blog regenerate via Claude, archives prior edition), and Editions (list archived versions). Endpoints: `GET /admin/service-insights`, `PATCH /admin/service-insights/{slug}`, `POST /admin/service-insights/{slug}/refresh`, `GET /admin/service-insights/{slug}/editions`.
- **Reading Analytics**: `POST /api/insights/track` {slug, event:view|share, platform} increments counters in `insight_stats`. Reader (`ServiceInsightPage`) fires a view on mount (useRef-guarded against StrictMode double-fire); ShareBar `onShare` records share + platform. `GET /api/admin/insights/analytics` returns total_reads/shares, top_read, top_shared, by_theme — rendered in the admin panel (bars + lists).
- Tested iteration_21: backend 18/18, frontend 100%, zero bugs. Post-test fixes: view-track dedupe (useRef), newsletter send moved to background task, admin load-error toast, sort-input validation.

## Iteration 6.9 (Jun 2026) — Featured Rotation, Subscriber Theme Preferences, Quote Sharing, Trending Strip
- **Featured Rotation**: featured insight is now a QUEUE in app_meta (`queue`, `index`). The star toggle (`POST /admin/service-insights/{slug}/feature`) adds/removes a slug; `GET /admin/featured-queue` lists it with a `current` flag; `_rotate_featured()` advances the index weekly (Mondays 06:00 IST in scheduler). `GET /service-insights-featured` returns the current queued item (fallback freshest). Edits preserve the live slug so rotation doesn't jump mid-week. Admin panel shows a "Featured rotation" card (featured-queue-panel) with a Live marker.
- **Subscriber Theme Preferences**: `PrefsIn.themes` + `INSIGHT_THEMES` (8 themes). `GET/POST /newsletter/preferences` now carry `all_themes`/`selected_themes` (stored as `interests_themes`). Preferences page (`/preferences`) has an "Insight themes" pill selector (pref-theme-*). `_send_insights_newsletter` tailors each subscriber's email via `_pick_for_subscriber` (prefers chosen themes, tops up with freshest).
- **Quote Sharing**: "Share SK's take" button (share-sk-take) in the reader's SK Insights callout copies a LinkedIn-ready quote (take + attribution + link) to clipboard and opens LinkedIn; records a share with platform "quote".
...
## Iteration 7.1 (Jun 2026) — Trending Flame Badge, Theme Landing Pages, Reader Momentum Bar, Recap Cadence
- **Trending Flame Badge**: `GET /service-insights-trending-slugs` (reads_week >= min_reads this week). Hub & theme-page cards show a lime "Trending" badge (lucide Flame, not emoji) for those slugs.
- **Theme Landing Pages**: `THEME_SLUGS` map + `GET /service-insights-themes` (counts+blurbs) and `GET /service-insights-theme/{slug}` (all insights in a theme). New `ThemePage.jsx` at `/insights/theme/:themeSlug` (shareable, Seo + ShareBar). Hub shows an "Explore by theme" chip row; reader "More in {theme}" links to the theme page (view-theme-page).
- **Reader Momentum Bar**: fixed top reading-progress bar (data-testid reading-progress) that fills on scroll; inline "Share SK's take" button at the end of the article (share-sk-take-end) in addition to the callout one.
- **Recap Cadence Control**: `GET/POST /admin/insights/recap-settings` (weekly|fortnightly|monthly|off) stored in app_meta; scheduler `_recap_due()` gates the Monday recap accordingly. Admin dropdown (recap-cadence) with optimistic update + race-guard.
- Tested iteration_24: backend 10/10, frontend ~95% (recap-cadence persist re-confirmed manually after clearing a test-induced IP throttle). Ops note: heavy automated traffic can trip the brute-force guard (`_banned_ips` in-memory + blocked_ips collection); clear the collection + restart backend to reset.

## Iteration 7.2 (Jun 2026) — Security audit fixes (SK Insights)
- **View-tracking dedup**: `POST /api/insights/track` now dedupes VIEW events per IP + slug per day via `insight_view_dedup` collection (TTL index expireAfterSeconds=172800). Repeat view hits from the same IP no longer inflate `reads`/`reads_week`, so the Trending Strip and Auto-Feature winner can't be gamed. Share events remain uncapped (as chosen).
- **Newsletter/digest HTML injection**: `emailer.py` now imports `html as _html` and HTML-escapes the subscriber/client `name` at every HTML injection point (insights newsletter, signals/library/sector digests, waitlist, session reminder, welcome, product purchase, abandoned-checkout, consent receipt). Plain-text `set_content` bodies unaffected.
- Verified via curl + Mongo inspection: 3 view hits → reads:1 (deduped); 2 share hits → shares:2 (uncapped); `render_insights_newsletter_html('<script>...')` output escapes the tag. Test data cleaned up.

## Iteration 7.3 (Sep 2026) — hCaptcha fix, Collateral & Downloads manager, Consent tick UX
- **hCaptcha production login fix**: backend HCAPTCHA_SECRET was the old site's secret (mismatch with the then-new sitekey). Updated backend/.env to the paired secret [value scrubbed]; added error-codes logging in verify_captcha(). Preview auto-passes captcha so pairing only surfaces on live domain — user must redeploy.
- **Collateral & Downloads manager** (Admin → "Collateral" tab, admin-tab-collateral): unified registry of every downloadable (14 strategy-tool PDFs, 3 digital products, 1 lead-magnet, learning-hub video ref, library audio ref) seeded into `collateral` collection. Per asset: shows where it's used (page/CTA), download current, MANUAL UPLOAD (Emergent Object Storage), AI GENERATE PDF (Claude via Emergent LLM key → collateral_pdf), EDIT metadata (title/desc/CTA/price), PUBLISH LIVE (swaps the served download in realtime, honored by /strategy-tools/{slug}.pdf, /strategy-tools-bundle.pdf, /blueprint/starter.pdf via _managed_collateral_response), optional NOTIFY subscribers, and ADD/DELETE custom collateral. Files: storage_helper.py, collateral.py, collateral_pdf.py, CollateralAdmin.jsx. Endpoints under /api/admin/collateral + public /api/collateral/{id}/file.
- **Fixed pre-existing bug**: strategy_pdf.build_tool_pdf() never called doc.build()/returned bytes — individual strategy-tool PDF downloads were 0 bytes. Now fixed.
- **Consent UX**: ConsentGate checkbox (consent-checkbox) is now enabled immediately — a simple tick, no need to open/read Terms & Privacy first (docs still optionally openable). Applied site-wide (login/register/renewal).
- Tested iteration_25: backend 100% (9/9), frontend 100%, zero critical/minor issues. Added Escape-key close on collateral modals.

## Iteration 7.4 (Sep 2026) — Collateral: Download Analytics, Gated Downloads, Bulk AI Refresh
- **Download Analytics**: every public download endpoint (/strategy-tools/{slug}.pdf, /strategy-tools-bundle.pdf, /blueprint/starter.pdf, /collateral/{id}/file) increments cumulative `downloads` + weekly `downloads_week` on the collateral doc. Collateral tab shows per-item download badges, a total, and a "Top downloaded" leaderboard (GET /api/admin/collateral now returns total_downloads + leaderboard).
- **Gated Downloads**: per-item admin toggle `gated` (default TRUE for strategy tools/bundle — preserving existing email capture; FALSE for products/videos/audio). POST /api/admin/collateral/{id}/gate flips it. Enforcement: gated free-download endpoints return 403 {detail:"gated"} unless the visitor has a `dl_gate` cookie. POST /api/collateral/unlock {name,email,source} captures the email into db.subscribers (nurture funnel) + sets a 30-day httpOnly dl_gate cookie. GET /api/strategy-tools returns `gated` per tool; StrategyToolkit.jsx opens the email modal for gated tools (modal now also calls /collateral/unlock) and downloads directly for open ones.
- **Bulk AI Refresh**: POST /api/admin/collateral/ai-refresh-all runs a background job that AI-generates + publishes fresh PDFs for all 14 strategy tools (409 if already running); GET /api/admin/collateral/ai-refresh-status reports {running,total,done}. Collateral tab has a "Regenerate all toolkit PDFs" button + live progress bar (polls).
- Tested iteration_26: backend 9/9, frontend 100%, zero issues.

## Iteration 7.5 (Sep 2026) — Security review of Collateral surface (3 fixes)
- **Gate bypass (HIGH) fixed**: a GATED tool with an uploaded managed file was downloadable unguarded at /api/collateral/{id}/file. serve_collateral_file now enforces the dl_gate cookie (403 "gated") when the item is gated. Verified: 403 without cookie, 200 with.
- **Unlock abuse (HIGH) fixed**: POST /api/collateral/unlock is public and no longer sends any email (previously could email arbitrary addresses = email-bombing vector). It now only sets the dl_gate cookie + quietly upserts the email (validated via EMAIL_RE, length-capped) into db.subscribers. Welcome emails remain owned by the captcha-gated /nurture/subscribe path.
- **Download-count inflation (MEDIUM) fixed**: _record_collateral_download now dedupes per IP+item+day via collateral_dl_dedup (TTL 48h), so the analytics/leaderboard can't be gamed by hammering endpoints. Applied across all public download endpoints (tool PDFs, bundle, starter, collateral file). Verified: 3 hits → 1 count.
- Self-tested via curl (backend only; no frontend change). Test data cleaned up.

## Iteration 7.6 (Sep 2026) — Security review: client-IP spoofing hardening
- **X-Forwarded-For spoofing (HIGH) fixed**: _client_ip() previously trusted the left-most, client-forgeable X-Forwarded-For value. Since production is fronted by Cloudflare, an attacker could spoof XFF to bypass the global rate limiter (100/10s), login lockout, IP bans, and download/insights dedup, and evade bans. Now prioritizes Cloudflare's authoritative CF-Connecting-IP, then True-Client-IP, then falls back to XFF/X-Real-IP/socket for the no-CF preview env (preview behavior unchanged). This restores integrity of ALL per-IP protections (brute-force, flood ban, gated-download dedup, insights view dedup, unlock throttle).
- Reviewed rest of recent collateral/consent surface: unlock endpoint already hardened in 7.5 (no email send, validated+capped writes) and is covered by the global flood limiter now that IP is reliable; file upload is admin-only, size-capped, uuid-pathed and served as attachment; gate enforcement + dedup verified in 7.5. No further changes needed.
- Self-tested via curl (login 200; CF-Connecting-IP honored; backend healthy). Backend-only change.

## Iteration 7.7 (Sep 2026) — Route safety + obscured admin path + signed URL ids
- **Error/expiry -> login**: axios response interceptor (lib/api.js) redirects to /login on 401 for admin/dashboard paths; /auth/me hydration got a 12s timeout so ProtectedRoute never hangs. friendlyAuthError() shows "server was briefly busy, please retry" for 5xx/non-JSON (Cloudflare) blips instead of raw text (Login/Register).
- **Obscured admin path (a)**: admin console moved from /admin to ADMIN_PATH='/sk-control-92f4a7e1' (frontend src/config.js + backend server.py kept in sync; Google Calendar OAuth redirects updated). /admin now <Navigate to='/login'>. Nav link, post-login redirect, PageTracker, and the 401 interceptor all reference ADMIN_PATH.
- **Signed URL ids (b)**: audit found all ids are UUIDs and emails already tokenized. Remaining bare-capability URLs now use signed tokens: blueprint download -> GET /api/blueprint/download?token=_download_token(order_id) (legacy /blueprint/download/{order_id} kept for old links); abandoned-cart resume link -> /resume/_resume_token(bid), decoded by /api/payments/resume/{bid} (token-or-raw). Hides raw ids + tamper-proofs the links.
- Tested iteration_28: backend 100%, frontend 100%, zero critical/minor issues.

## Iteration 7.8 (Sep 2026) — hCaptcha verify hardening + on-screen diagnosis
- verify_captcha() now calls hCaptcha siteverify with ONLY the required {secret, response} (removed optional sitekey + remoteip params which can add sitekey-secret-mismatch / remoteip-mismatch failures). The recent CF-Connecting-IP change had started sending remoteip=real-client-ip, a likely cause of the post-deploy captcha failures.
...
- Frontend & backend sitekeys confirmed matching (see .env for current value). If it still fails post-redeploy, the bracketed code identifies whether it's an account mismatch (sitekey-secret-mismatch), stale deployed secret (invalid-input-secret), or domain/token issue (invalid-input-response).
...
## Security audit (2026-09-03) — Magic Link hardening
- **[CRITICAL — FIXED] Host/Origin injection in magic link**: _site_base() previously trusted the request Origin/Host header when building the emailed sign-in link. An attacker knowing an admin email could POST /auth/magic-link with a spoofed Origin, pointing the real admin\047s emailed link to attacker.com and exfiltrating the single-use token. Fixed: _site_base() now only uses the request host when it is on a trusted allowlist (_magic_allowed_hosts from prod domains + WEBAUTHN_ORIGIN + GOOGLE_CALENDAR_REDIRECT_URI + optional MAGIC_LINK_BASE); otherwise falls back to PUBLIC_SITE. Verified: spoofed attacker.com -> canonical site; preview/prod hosts allowed.
...
## Security audit (2026-09-03, round 2) — Magic link scanner-safe redesign
- **[MEDIUM — FIXED] Email link-scanner token/JWT exposure.** The old GET /auth/magic-login consumed the token and returned the admin JWT in the redirect Location header. Corporate email scanners (Safe Links/Proofpoint/AV) auto-fetch links and would (a) burn the single-use token before the admin clicked and (b) receive a valid 7-day admin JWT. Redesigned to a two-step, prefetch-safe flow: GET renders a click-to-confirm interstitial (noindex, does NOT consume the token, no JWT); a real human click POSTs to /auth/magic-exchange, which single-use-consumes + mints the JWT in the JSON body (never in any URL). Scanners do not POST. Interstitial stores sk_token in localStorage (same origin) and redirects to the admin path.
- **Audit trail**: every successful magic-link sign-in now writes an audit_log entry (action=admin_magic_login, actor+target=admin email, IP).
- Verified via curl: GET renders interstitial + leaves used=False; POST exchange #1 mints JWT, #2 -> 400 (single-use); GET invalid -> /login?magic=invalid; audit entry written. Login.jsx dead #magic_token handler removed; frontend compiles clean.

## Security audit (2026-09-03, round 3) — Captcha bypass via header spoofing [CRITICAL — FIXED]
- **Finding**: _is_preview_host() derived preview-vs-prod from x-forwarded-host / origin / host. Origin (and often X-Forwarded-Host) are client-spoofable, so a production request carrying `Origin: https://x.preview.emergentagent.com` made verify_captcha() return True with NO verification -> full captcha bypass on login, register, newsletter, consultation, and the Google captcha-gate (enables bot/credential-stuffing/spam).
- **Fix**: _is_preview_host() now reads ONLY the Host header (the real routing host set by ingress/Cloudflare; a browser cannot forge it to reach the production origin). X-Forwarded-Host and Origin are no longer trusted for this decision — same trust model already used for IPs via CF-Connecting-IP. Probe confirmed: preview Host = *.preview.emergentcf.cloud (still matches PREVIEW_HOST_MARKERS -> auto-pass preserved); prod Host = real domain -> full verify. Ingress also overwrites spoofed Origin/XFH on preview.
- Verified: captcha-gate on preview still ok:true; unit test — prod host + spoofed origin/xfh => False (no bypass), preview internal host => True.

## 2026-09-05 — Captcha Health widget + AI Global Benchmark panel
- **Captcha Health** (Admin → Overview card, data-testid captcha-health-card): GET /api/admin/captcha/health reports sitekey + secret status (valid/invalid/test via a dummy siteverify). Live pairing test: admin solves the REAL hCaptcha (CaptchaHealth.jsx renders HCaptcha with the configured sitekey) -> POST /api/admin/captcha/verify-test verifies the real token against the secret -> green "Pairing verified" or red "sitekey-secret-mismatch" with the exact code. Only definitive way to catch a mismatch pre-visitor. Verified via curl + screenshot.
- **Global Best-Practices Benchmark** (Admin → Benchmark tab, data-testid benchmark-panel): AI (Claude Sonnet 4.6 via Emergent LLM key) benchmarks the site vs McKinsey/a16z/HBR/Naval/Simon Sinek/Stratechery etc. ASYNC to dodge the ~55s LLM call vs 60s ingress timeout: POST /api/admin/benchmark/generate sets status=running + asyncio.create_task(_run_benchmark), returns immediately; GET /api/admin/benchmark returns {status,summary,recommendations[]}; frontend polls every 4s. Stored in db.site_benchmark {_id:latest}. 10-14 prioritised (P0/P1/P2) recs across 8 categories, each with recommendation/benchmark/impact. Verified running->done in ~45s, 13 recs, screenshot confirms render.
- New files: frontend/src/components/CaptchaHealth.jsx, BenchmarkAdmin.jsx. New admin tab "benchmark" added to AdminDashboard.jsx.

## 2026-09-05 — Acted on Benchmark P0s (homepage conversion + trust)
- **Hero CTA carousel** (Hero.jsx): single dominant CTA rotating through 4 outcome phrasings every 3.2s (Book Your Strategy Session / Map Your Growth Strategy / Get a Custom Growth Plan / Book a Premium 1:1) via framer-motion AnimatePresence; secondary "Explore Services" demoted to a subtle text link (singular-CTA best practice). data-testid hero-cta-rotating, hero-services-cta.
...
## 2026-09-05 — Lazy captcha loading
- Captcha.jsx now defaults to lazy=true: renders a lightweight "Verify you're human" placeholder (data-testid captcha-activate) and mounts the real hCaptcha widget ONLY when the visitor first focuses the surrounding form (focusin on closest <form> / nearest field-container, once) or clicks the placeholder. findFieldContainer() walks up to the form or a container holding input/textarea/select (covers NewsletterSignup where Captcha sits just outside </form>).
- Effect: hCaptcha script/challenge is no longer fetched on every page view — only on real form engagement — cutting load and the rate-limit note risk across all forms (consultation, login, register, lead-magnet, checkout, newsletter, toolkit). Admin CaptchaHealth uses HCaptcha directly and is unaffected. Verified via screenshot: 0 hcaptcha iframes before focus, widget mounts after focusing a field.

## 2026-09-05 — The SK Strategy Brief (AI podcast, Content P0)
- **New**: AI-generated weekly podcast. Script via Claude Sonnet 4.6 (first-person SK, opens EXACTLY "I'm Sudarshan Karweer, and this is The SK Strategy Brief..."), narrated by OpenAI TTS voice **onyx** (tts-1-hd, chunked <4096 chars, mp3 concat), stored in Emergent Object Storage. Backend module: backend/podcast.py.
- **Endpoints** (server.py): public GET /api/podcast/episodes, /episodes/{id}, /episodes/{id}/audio (streams mp3), /intro-audio; admin GET /admin/podcast/episodes, POST /generate (async: doc status generating->published, asyncio.create_task), /schedule, /intro (upload clip), {id}/publish|unpublish, DELETE {id} & /intro. Collections: podcast_episodes, app_meta(_id podcast_schedule / podcast_intro).
- **Weekly auto**: _digest_scheduler() hourly loop generates one episode Mon ~10:00 IST if none for the ISO week and schedule enabled (default on). Manual generate button too. Topics auto-picked from expanded themes (energy transition/BESS/H2/climate finance + strategy/structure/people/processes/KPIs/scorecards/winning cos/transition mgmt/hiring) or admin topic box.
- **Frontend**: public /pages/PodcastPage.jsx at route /podcast (nav link "Podcast"), audio player (plays optional uploaded SK intro clip then episode), takeaways, transcript expander. Admin /components/PodcastAdmin.jsx = new "podcast" tab (generate/topic, weekly toggle, intro upload, list w/ status+publish/unpublish/delete, players).
- Verified: curl generate->published in ~48s, 5.4MB onyx mp3 streams (audio/mpeg), single endpoint returns transcript starting with SK intro; screenshots of public page + admin tab. No new pip deps (uses emergentintegrations OpenAITextToSpeech). Generation billed to Universal Key.

## Security Audit + Hardening (2026-06-05)
- **Full security audit** performed across backend (server.py, auth.py, podcast.py) + frontend. Result: strong baseline (bcrypt+salt, env-only JWT HS256 fail-fast, no injection/eval sinks, no XSS sinks, Mongo projections strip _id/password_hash, Razorpay webhook HMAC-verified, comprehensive SecurityGuard middleware, strict admin allowlist).
- **FIXED — HIGH: CORS wildcard with credentials**. `allow_credentials=True` + `CORS_ORIGINS=*` caused Starlette to reflect ANY origin with Allow-Credentials on cookie-bearing requests (session_token/captcha_gate/vpn_totp/vault cookies exposed cross-site). Set `CORS_ORIGINS="https://www.sudarshankarweer.com,https://sudarshankarweer.com,https://energy-strategy-hub.preview.emergentagent.com"` in backend/.env. Verified via curl on localhost:8001: allowed origins reflected, `evil.example.com` refused (no ACAO), no ACAO without Origin. (Preview ingress still injects a credential-less `ACAO: *` — harmless; app-level allowlist is what applies in production.)
- **ADDED — Content-Security-Policy**. Meta CSP in frontend/public/index.html (protects the actual rendered pages, since FastAPI only serves /api) + matching CSP_POLICY header on backend /api responses (server.py, ~line 6538). Directives: default-src 'self'; object-src 'none'; base-uri 'self'; frame-ancestors 'self'; script-src allows self+inline+eval (CRA runtime) + assets.emergent.sh + ap.emergent.sh + static.cloudflareinsights.com + checkout.razorpay.com + *.hcaptcha.com + youtube; frame-src allows hcaptcha/razorpay/youtube; img/media/connect kept permissive (https:/wss:) to avoid breakage. Verified: 0 CSP violations on home/podcast/login pages; hCaptcha iframe, audio player, images all load.
...
## Captcha migration: hCaptcha → Cloudflare Turnstile (2026-06-05)
- User reported hCaptcha continuously erroring → switched the whole site to **Cloudflare Turnstile** (via verified integration playbook). Free, privacy-first, reliable, usually invisible.
- Backend: `verify_captcha()` now posts to `challenges.cloudflare.com/turnstile/v0/siteverify` with `TURNSTILE_SECRET` + `remoteip`; preview hosts still auto-pass; lenient while secret is a Turnstile TEST secret. `/api/public-config` returns `turnstile_sitekey` + `captcha_provider:"turnstile"`. Env added: `TURNSTILE_SITEKEY`/`TURNSTILE_SECRET` (currently always-pass TEST pair `1x...AA`).
- Frontend: `Captcha.jsx` rewritten for Turnstile (explicit render, lazy-on-focusin, dark theme, retry loop, reset/reload). SAME interface (`onVerify`/`onExpire`/`ref.resetCaptcha()`) so all 12 consuming forms were untouched. `config.js` adds `getTurnstileSitekey()` + caches `turnstile_sitekey`. `REACT_APP_TURNSTILE_SITEKEY` added to frontend/.env. Production hostnames use the real sitekey; preview/localhost use the TEST sitekey. testids: `captcha` (wrapper), `turnstile-widget`, `captcha-activate`, `captcha-reload`.
- CSP (index.html meta + backend header) now allows `https://challenges.cloudflare.com` in script-src + frame-src.
- Verified: end-to-end admin login through the UI (widget auto-solves → consent → Sign in → admin dashboard) + backend curl 200. Testing agent iteration_29: frontend 100%, 0 bugs, 0 CSP violations; Turnstile renders + auto-solves on Register/Newsletter/Consultation, Newsletter submits.
...
- **RESOLVED (2026-06-05)**: user enabled the `text_to_speech` permission. Verified end-to-end — generated episode "Stop Chasing Revenue. Start Chasing Cash." in ~40s with NO OpenAI fallback logged → narrated in SK's ElevenLabs cloned voice (4.77MB mp3); corporate music intro plays first on the public player. All three features fully working.
