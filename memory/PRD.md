...
- **hCaptcha production login fix**: backend HCAPTCHA_SECRET was the old site's secret (mismatch with new sitekey 73c8f63e…). Updated backend/.env to the paired secret ES_8a9bc64f…; added error-codes logging in verify_captcha(). Preview auto-passes captcha so pairing only surfaces on live domain — user must redeploy.
